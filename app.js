...
function convertToEmbedUrl(rawUrl, timeSeconds = 0) {
    if (!rawUrl) return "";
    ...
    if (cleanUrl.includes("drive.google.com")) {
        ...
        if (fileId) {
            ...
            if (timeSeconds > 0) url += `#t=${Math.floor(timeSeconds)}`;
            return url;
        }
    }
    ...
    if (videoId) {
        let tParam = timeSeconds > 0 ? `&start=${Math.floor(timeSeconds)}` : "";
        return `https://www.youtube.com/embed/${videoId}?rel=0&modestbranding=1&playsinline=1${tParam}`;
    }
    return cleanUrl;
}
...
async function loadCatalogData() {
    ...
    if (cachedData && cachedTime && (now - cachedTime < 10 * 60 * 1000)) {
        globalCatalogData = JSON.parse(cachedData);
        renderHomeCatalog();
        return;
    }
...
function playRandomTitle() {
    if (typeof globalCatalogData === "undefined" || globalCatalogData.length === 0) return;
    const showsMap = {};
    globalCatalogData.forEach(item => {
        if (!showsMap[item.title]) showsMap[item.title] = item;
    });
    const uniqueShows = Object.values(showsMap);
    ...
    seasonsSet.forEach(seasonNum => {
        ...
        card.innerHTML = `
            <div style="width: 100%; aspect-ratio: 2/3; overflow: hidden; background: #000;">
                <img src="${poster}" alt="${seasonNum}" loading="lazy" style="width: 100%; height: 100%; object-fit: cover;" onerror="this.src='https://via.placeholder.com/300x450/181818/ffffff?text='+encodeURIComponent('${seasonNum}')">
            </div>
    ...
    if (currentEpisodesViewMode === 'list') {
        ...
        episodesList.forEach(ep => {
            ...
            item.querySelector("button").onclick = () => {
                openMediaViewer(`${ep.title} - ${seasonLabelText} (${ep.episodeName})`, ep.videoUrl, ep.title, seasonNum, ep.episodeName, ep.posterUrl);
            };
        ...
        episodesList.forEach(ep => {
            ...
            card.innerHTML = `
                <div style="width: 100%; aspect-ratio: 16/9; background: #000; overflow: hidden; position: relative;">
                    <img src="${thumb}" alt="${ep.episodeName}" loading="lazy" style="width: 100%; height: 100%; object-fit: cover;" onerror="this.src='https://via.placeholder.com/300x169/181818/ffffff?text='+encodeURIComponent('Епизод ${ep.episode}')">
                </div>
...
function removeFromHistory(event, videoUrl) {
...
function renderContinueWatching() {
    ...
    history.forEach(item => {
        ...
        card.onclick = () => {
            openMediaViewer(`${item.title} - Сезон ${item.season} (${item.episodeName})`, item.videoUrl, item.title, item.season, item.episodeName, item.posterUrl);
        };
        ...
        card.innerHTML = `
            ...
            <div style="padding: 10px 12px;">
                ...
                <div style="color: #aaa; font-size: 0.75rem; margin-top: 3px; display: flex; justify-content: space-between;">
                    <span>Сезон ${item.season} • ${item.episodeName}</span>
                </div>
            </div>
        `;
